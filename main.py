...
def fetch_hgnc_data(gene_symbol: str) -> Dict[str, str]:
    ...
    """
    try:
        response = requests.get(f"{constants.HGNC_API}{gene_symbol}", headers={"Accept": "application/json"})
...
def fetch_ensembl_coordinates(gene_symbol: str) -> tuple:
    ...
    """
    try:
        response = requests.get(f"{constants.ENSEMBL_API}{gene_symbol}?expand=1", headers={"Content-Type": "application/json"})
        response.raise_for_status()
        data = response.json()
        
        hg38_coords = f"{data['seq_region_name']}:{data['start']}-{data['end']}"
        
        xref_response = requests.get(f"{constants.ENSEMBL_XREF_API}{gene_symbol}?external_db=HGNC", headers={"Content-Type": "application/json"})
...
def run(pmid: str, outfile: str, config: dict, verbose: bool = constants.DEFAULT_VERBOSE) -> None:
...
@click.command()
@click.option('--config_file', type=click.Path(exists=True), help=f"The configuration file for this project - default is '{constants.DEFAULT_CONFIG_FILE}'")
...
@click.option('--verbose', is_flag=True, callback=validate_verbose, help=f"Will print more info to STDOUT - default is '{constants.DEFAULT_VERBOSE}'.")
def main(config_file: str, infile: str, logfile: str, outdir: str, outfile: str, pmid: str, verbose: bool):
    """Retrieve gene info for PMID."""
    error_ctr = 0
