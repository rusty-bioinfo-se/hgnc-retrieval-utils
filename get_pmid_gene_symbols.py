...
def get_gene_symbols_from_pubtator(pmid):
    ...
    try:
        ...
        print(
            f"PubTator raw annotations for PMID {pmid}: {[ann for passage in data.get('passages', []) for ann in passage.get('annotations', [])]}"
        )
...
def get_gene_symbols_from_europe_pmc(pmid):
    ...
    try:
        ...
        print(
            f"Europe PMC raw annotations for PMID {pmid}: {data.get('annotations', [])}"
        )
        return list(gene_symbols)
...
def get_gene_symbols_from_scispacy(text):
    ...
    if nlp_bionlp:
        try:
            doc = nlp_bionlp(text)
            gene_symbols.update(
                ent.text for ent in doc.ents if ent.label_ == "GENE_OR_GENE_PRODUCT"
            )
